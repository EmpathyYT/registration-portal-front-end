...
const MOCK_MY_TEAM: Team = {
    ...
    status: 'Active',          // teams.status — project_status enum from DB
    min_users: 3,
    max_users: 4,
    introduction_link: 'https://docs.google.com/document/d/mock',
    supervisor_id: 'sup1',
    supervisor_name: 'Dr. Mohammed Ali',  // joined from users WHERE id = supervisor_id
};
...
// Query: SELECT teams.* FROM teams WHERE supervisor_id IS NULL (for student browsing)
...
// Query: SELECT invitations.*, users.full_name AS sender_full_name, users.university_id AS sender_university_id
//        FROM invitations JOIN users ON users.id = invitations.sender_user_id
//        WHERE invitations.receiver_user_id = $supervisorId
...
// Note: status 'Active'/'Recruiting' come from teams.status (project_status enum in DB)
const MOCK_SUPERVISED_TEAMS: SupervisedTeamData[] = [
    {
        team: {
            team_id: 1, project_title: 'University Course Registration System',
            status: 'Active',       // from teams.status enum
            min_users: 3, max_users: 4,
            introduction_link: 'https://docs.google.com/document/d/mock1',
            supervisor_id: SUPERVISOR_USER_ID, supervisor_name: SUPERVISOR_NAME,
        },
        members: [
            { team_id: 1, user_id: 'u1', full_name: 'Ammar Ahmad Sameed', university_id: '20221001', role: 'student', team_role: 'Team Leader' },
            { team_id: 1, user_id: 'u2', full_name: 'Basel Ibrahim',       university_id: '20221002', role: 'student', team_role: 'Backend Dev' },
    ...
    {
        team: {
            team_id: 2, project_title: 'AI-Powered Study Assistant',
            status: 'Active',       // from teams.status enum
            min_users: 2, max_users: 4,
            introduction_link: '',
            supervisor_id: SUPERVISOR_USER_ID, supervisor_name: SUPERVISOR_NAME,
        },
        members: [
    ...
    {
        team: {
            team_id: 3, project_title: 'Smart Campus Navigation System',
            status: 'Recruiting',   // from teams.status enum
            min_users: 4, max_users: 5,
            introduction_link: '',
            supervisor_id: SUPERVISOR_USER_ID, supervisor_name: SUPERVISOR_NAME,
        },
        members: [
...
// These are teams with no supervisor — a supervisor can take any of them
...
export default function ProjectDashboard({ onSwitchPage, onLogout, isDark, onToggleDark, userRole }: ProjectDashboardProps) {
    ...
    // Invitations — separate lists per role
    ...
    // Teams with no supervisor — fetched via SELECT * FROM teams WHERE supervisor_id IS NULL
    ...
    const handleCreateTeam = (projectTitle: string) => {
        ...
        setNotice({ type: 'success', message: `Team "${projectTitle}" was created.` });
    };
    ...
    // ── Supervisor: stop supervising a team ───────────────────
    ...
    const handleStopSupervising = () => {
        ...
        if (window.confirm(`Stop supervising "${title}"?`)) {
            ...
            // Put the team back into the unsupervised pool (supervisor_id cleared)
    ...
    // ── Supervisor: take supervision of an unsupervised team ───
    ...
    const handleSuperviseTeam = (teamId: number) => {
        ...
        const newEntry: SupervisedTeamData = {
            ...
            members: [], // real app: fetch team_members WHERE team_id = $teamId
        };
        setSupervisedTeams(prev => [...prev, newEntry]);
        setUnsupervisedTeams(prev => prev.filter(t => t.team_id !== teamId));
        setSelectedSupTeamId(teamId);
        setNotice({ type: 'success', message: `You are now supervising "${team.project_title}".` });
    };

    // ── Shared member handlers ─────────────────────────────────
    const getActiveTeam = (): Team =>
        isSupervisor
            ? supervisedTeams.find(t => t.team.team_id === selectedSupTeamId)?.team ?? MOCK_MY_TEAM
            : myTeamData;
        ...
        <div className="min-vh-100 pb-5" style={{ paddingTop: '5.5rem' }}>
            ...
            <div className="container" style={{ maxWidth: '1100px' }}>
                ...
                {isSupervisor ? (
                    /* ══ SUPERVISOR VIEW ══════════════════════════ */
                    <>
                        {/* Team selector — only shown when supervising at least one team */}
                        ...
                        {/* Invitations feed — supervisors receive invites from team leaders */}
                        <div className="section-enter">
                            <InvitationsFeed
                        ...
                        {supervisedTeams.length > 0 ? (
                            ...
                            <div className="section-enter mb-4">
                                <div className="alert border-0 shadow-sm rounded-4 p-5 text-center">
                                    ...
                                    <p className="text-muted mb-0 small">Browse the teams below and click "Supervise This Team" to get started.</p>
                                </div>
                            </div>
                        )}

                        {/* Teams without a supervisor — supervisor can take any of them */}
                        {/* Query: SELECT * FROM teams WHERE supervisor_id IS NULL */}
                        <div className="section-enter">
                            <AvailableTeamsGrid
                    ...
                    <>
                        <div className="text-center mb-5 fade-up">
                            <h2 className="fw-bolder mb-1 page-title" style={{ letterSpacing: '-0.5px' }}>Project Management</h2>
